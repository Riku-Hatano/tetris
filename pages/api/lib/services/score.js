    ...
    case "POST":
      ...
      switch (req.body.purpose) {
        case "send":
          ...
          managementClient
            .getSpace("jivp4q6rn93f")
            .then((space) => space.getEnvironment("master"))
            .then((environment) =>
              environment.createEntry("tetrisscores", entry),
            )
            .then((entry) => {
              entry.publish();
          ...
          break;
        case "pick":
          client.getEntries({ content_type: "tetrisscores" }).then((resp) => {
        ...
        case "pickHighest": {
          ...
          client
            .getEntries({ content_type: "tetrisscores" })
            .then(
              (resp) => {
                const highest = {
                  uname: "",
                ...
                if (bodyData !== null) {
                  resp.items.map((scoreData) => {
                    ...
                    if (
                      bodyData.uid === scoreData.fields.uid &&
                      scoreData.fields.score > yourHighest.score
                    ) {
                      yourHighest.score = scoreData.fields.score;
                    }
                    if (
                      today === scoreData.fields.playdate.slice(0, 10) &&
                      scoreData.fields.score > todayHighest.score
                    ) {
