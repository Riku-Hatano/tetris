...
for (let i = 0; i < 3; i++) {
  //Only first time, select four tetriminoes and show three of them at nextBlock, which is right side of Tetris game.
...
const CreateRect = () => {
  ...
  const newBlock = new BlockClass(
    Setting.block.size,
    Setting.block.initialX,
    Setting.block.initialY,
    GameStatus.blockCounter++,
    GameStatus.nextBlock[0],
  );
  newBlock.setColor();
  GameStatus.block = newBlock;
  GameStatus.nextBlock.shift();
